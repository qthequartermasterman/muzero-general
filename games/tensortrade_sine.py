...
from .trading_envs import create_sine_env
...
class MuZeroConfig:
    def __init__(self):
        ...
        self.seed = hash(datetime.datetime.now()) % 2**32
        self.max_num_gpus = None  # Fix the maximum number of GPUs to use. It's usually faster to use a single GPU (set it to 1) if it has enough memory. None will use every GPUs available

        ### Game
        self.observation_shape = (1, 25, 5)  # Dimensions of the game observation, must be 3D (channel, height, width). For a 1D array, please reshape it to (1, 1, length of array)
        self.action_space = list(range(181))  # Fixed list of all possible actions. You should only edit the length
        ...
        self.root_exploration_fraction = .4
...
class Game(AbstractGame):
    ...
    def __init__(self, seed=None):
        self.env_type = create_sine_env
        self.env = self.env_type({"window_size": 25})
        if seed is not None:
            self.env.seed(seed)
    ...
    def reset(self):
        ...
        """
        if self.env.clock.step != self.env.clock.start:
            self.env.render()
        self.env = self.env_type({"window_size": 25})
    ...
    def render(self):
        ...
        """
        # if self.env.stopper.stop(self.env):  # Only render if we're done.
        self.env.render()
    ...
    def action_to_string(self, action_number):
        ...
        """
        return f'{action_number}. {str(self.env.action_scheme.actions[action_number])}'
